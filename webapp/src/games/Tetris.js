let Engine = {
  RuntimeEnvironment (Module, exposedLibs) {
		...
		var asmLibraryArg = {
			...
			"qi": _emscripten_get_num_gamepads,
			"pi": _emscripten_get_pointerlock_status,
			"oi": _emscripten_glActiveTexture,
			"ni": _emscripten_glAttachShader,
			"mi": _emscripten_glBeginQuery,
			"li": _emscripten_glBeginQueryEXT,
			"ki": _emscripten_glBeginTransformFeedback,
			"ji": _emscripten_glBindAttribLocation,
			"ii": _emscripten_glBindBuffer,
			"hi": _emscripten_glBindBufferBase,
			"gi": _emscripten_glBindBufferRange,
			"fi": _emscripten_glBindFramebuffer,
			"ei": _emscripten_glBindRenderbuffer,
			"di": _emscripten_glBindSampler,
			"ci": _emscripten_glBindTexture,
			"bi": _emscripten_glBindTransformFeedback,
			"ai": _emscripten_glBindVertexArray,
			"$h": _emscripten_glBindVertexArrayOES,
			"_h": _emscripten_glBlendColor,
			"Zh": _emscripten_glBlendEquation,
			"Yh": _emscripten_glBlendEquationSeparate,
			"Xh": _emscripten_glBlendFunc,
			"Wh": _emscripten_glBlendFuncSeparate,
			"Vh": _emscripten_glBlitFramebuffer,
			"Uh": _emscripten_glBufferData,
			"Th": _emscripten_glBufferSubData,
			"Sh": _emscripten_glCheckFramebufferStatus,
			"Rh": _emscripten_glClear,
			"Qh": _emscripten_glClearBufferfi,
			"Ph": _emscripten_glClearBufferfv,
			"Oh": _emscripten_glClearBufferiv,
			"Nh": _emscripten_glClearBufferuiv,
			...
			"L": _glDeleteShader,
			"K": _glDeleteTextures,
			"ra": _glDeleteVertexArrays,
			"qa": _glDepthFunc,
			"R": _glDepthMask,
			"w": _glDisable,
			"u": _glDisableVertexAttribArray,
			"z": _glDrawArrays,
			"pa": _glDrawArraysInstanced,
			"Va": _glDrawBuffers,
			"Q": _glDrawElements,
			"Ca": _glDrawElementsInstanced,
			"E": _glEnable,
			...
			"Ua": _glRenderbufferStorageMultisample,
			"ua": _glScissor,
			"$a": _glShaderSource,
			"D": _glTexImage2D,
			"Ta": _glTexImage3D,
			...
			"oa": _glUseProgram,
			"x": _glVertexAttrib4f,
			"X": _glVertexAttrib4fv,
			"A": _glVertexAttribDivisor,
			"Za": _glVertexAttribI4ui,
...
(function () {
  ...
  Engine = function Engine() {
    ...
    this.preloadFile = function (pathOrBuffer, destPath) {
      ...
      if (pathOrBuffer instanceof Uint8Array) {
        ...
        return loadPromise(pathOrBuffer, preloadProgressTracker).then((xhr) => {
					preloadedFiles.push({
    ...
    function animateProgress() {
      ...
      [loadingFiles, preloadProgressTracker].forEach((tracker) => {
  ...
  Engine.load = function (newBasePath) {
    ...
    if (engineLoadPromise === null) {
      ...
      engineLoadPromise = loadPromise(`${basePath  }.${  wasmFilenameExtensionOverride || 'wasm'}`).then((xhr) => {
				return xhr.response;
			});
      engineLoadPromise = engineLoadPromise.catch((err) => {
  ...
  function loadXHR(resolve, reject, file, tracker) {
    ...
    ['loadstart', 'progress', 'load', 'error', 'abort'].forEach((ev) => {
